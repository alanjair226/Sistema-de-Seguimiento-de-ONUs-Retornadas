...
const App = () => {
    ...
    const [desechado, setDesechado] = useState([]);
    ...
    const loadTableData = async () => {
        const apiUrl = `${process.env.REACT_APP_URL_API_GESTION}/onus/sinmotivo/estado`;
        try {
            const response = await fetch(apiUrl);
            const data = await response.json();
            setDesechado(data.filter((item) => item.estado === 'desechado'));
            setAlmacen(data.filter((item) => item.estado === 'almacen'));
        } catch (error) {
    ...
    const handleVerify = async (sn, motivoId, modeloId) => {
        try {
            ...
            fetchOrder();
            fetchHistory(sn);
            loadTableData(); // Actualizamos las tablas automáticamente
        } catch (error) {
    ...
    useEffect(() => {
        ...
        fetchMotivos();
        fetchModelos();
        loadTableData(); // Cargamos los datos de la tabla al inicio
    }, []);
    ...
    return (
        <div className="flex flex-col items-center min-h-screen bg-gray-100 p-4">
            ...
            <ONUTable desechado={desechado} almacen={almacen} loadTableData={loadTableData} />
        </div>
    );
};
